...
class Parser {
  ...
  parseProgram() {
    ...
    while (this.current.type !== TOKENS.EOF) {
      ...
      } else if (this.current.type === TOKENS.PUBLIC && 
                 (this.pos + 1 < this.tokens.length && 
                  this.tokens[this.pos + 1] && 
                  (this.tokens[this.pos + 1].type === TOKENS.CLASS || this.tokens[this.pos + 1].type === TOKENS.ABSTRACT))) {
        // Skip `public` before class/abstract class
        this.eat(TOKENS.PUBLIC);
...
if (
  ...
  continue;
      ...
      // Main method: public [static] void main() or main(String[] args) { ... }
if (
  access === 'public' &&
  (this.current.type === TOKENS.VOID ||
   (this.current.type === TOKENS.STATIC && this.tokens[this.pos + 1] && this.tokens[this.pos + 1].type === TOKENS.VOID))
) {
  // Consume optional 'static'
  if (this.current.type === TOKENS.STATIC) this.eat(TOKENS.STATIC);
  if (
    this.current.type === TOKENS.VOID &&
    this.tokens[this.pos + 1] && this.tokens[this.pos + 1].type === TOKENS.MAIN
  ) {
    this.eat(TOKENS.VOID);
    this.eat(TOKENS.MAIN);
    this.eat(TOKENS.LPAREN);
    // Consume optional params: String[] args  or  String... args
  ...
  // Not main — fall through to method parsing
  if (isStatic) {
    methods.push(this.parseMethodDeclaration(access, true));
  } else {
    methods.push(this.parseMethodDeclaration(access, false));
  }
  continue;
}
